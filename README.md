...
The suite is 1123 tests. CI runs:

```bash
python -m unittest discover -s tests -p "test_*.py"
```

A clean run reports `Ran 1123 tests`.

1123 is the number collected under `tests` by the documented runner. The
repository root collects 1128 because a bare pytest collection also finds five
