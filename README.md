...
The suite is 627 tests. The runner is `unittest`, which is what CI executes:

```bash
python -m unittest discover -s tests -p "test_*.py"
```

627 is the number `python -m pytest --collect-only -q tests` reports and the
number `unittest` reports as `Ran 627 tests`, so it is reproducible from a clean
...
root collects 632, because it picks up the five legacy checks in
