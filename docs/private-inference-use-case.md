...
KaroX already blocks common secret paths, confines file access to the selected repository, authenticates protected endpoints, and exposes explicit access modes. Remote Git push and prerelease publication are not standing agent capabilities: they require dedicated exact-action, one-shot user approval. Those controls protect the local bridge. They do **not**, by themselves, prove that an external inference provider cannot inspect model inputs.
