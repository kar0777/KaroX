...
That reports `Ran 607 tests` with 3 environment/platform skips on Windows, and it
is the runner CI executes. 607 is also what `python -m pytest --collect-only -q
...
-q` at the repository root collects 612, because it also picks up the five legacy
