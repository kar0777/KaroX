...
That reports `Ran 604 tests` with 3 environment/platform skips on Windows, and it
is the runner CI executes. 604 is also what `python -m pytest --collect-only -q
...
-q` at the repository root collects 609, because it also picks up the five legacy
