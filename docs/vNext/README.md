...
That reports `Ran 595 tests` with 3 environment/platform skips on Windows, and it
is the runner CI executes. 595 is also what `python -m pytest --collect-only -q
...
-q` at the repository root collects 600, because it also picks up the five legacy
