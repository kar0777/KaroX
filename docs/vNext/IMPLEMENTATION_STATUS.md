...
| 10 — benchmark/readiness | Complete | Current 607-test suite plus KB-HYBRID-01..10 functional run records |
...
- Current local evidence is 607 tests with three platform skips on Windows, run as
  `python -m unittest discover -s tests -p "test_*.py"` — the runner CI uses.
  607 is what `python -m pytest --collect-only -q tests` reports too; a pytest
