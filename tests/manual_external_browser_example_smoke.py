"""Mandatory example.com external-browser smoke, separate from parallel agent checks."""
...
class ExternalBrowserExampleSmoke(unittest.TestCase):
    def test_example_tabs_snapshot_screenshot_and_signup_form(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for probe in Path(__file__).parent.glob(
                ".karox_external_browser_write_probe*.txt"
            ):
                probe.unlink()
            manager = SecureBrowserSessionManager(
                ArtifactStore("external-example-smoke"),
                BrowserAccessPolicy(
                    session_id="external-example-smoke",
                    localhost=True,
                    external_https=True,
                    headed=False,
                    user_takeover=False,
            ...
            try:
                ...
                screenshot = manager.screenshot(
                    {"full_page": True, "name": "example-pinned-proxy-smoke"}, 30
                )
                ...
                diagnostics = web_bridge_diagnostics(
                    config, session_id="external-example-smoke"
                )
                ...
                print(
                    "KAROX_EXTERNAL_BROWSER_EXAMPLE_SMOKE="
