...
Если ChatGPT-клиент не поддерживает MCP elicitation для опасной операции вроде
`git.push`, KaroX возвращает короткоживущую ссылку на собственную страницу
подтверждения. Браузер, который уже успешно проходил KaroX OAuth approval после
включения trusted-device механизма, увидит только кнопку подтверждения — пароль
повторно не нужен. Новый браузер или устройство по-прежнему потребует текущий
`OAuth approval password`; после успешного ввода устройство станет доверенным.
Подтверждение всё равно привязано к точной операции и действует один раз.
