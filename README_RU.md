...
Команда устанавливает последний стабильный релиз KaroX, Python-зависимости, команду `karox` и системный launcher. Эту же команду можно повторно использовать для переустановки или обновления.
...
Запуск Project Flight Deck:

```bash
karox
```

Запуск сразу с выбранным Notion:

```bash
karox notion
```

## Что нового в KaroX 3.12

- **Control Center:** браузерная панель живой сессии через `karox dashboard`.
- **Нормальное самообновление:** проверка и установка стабильных релизов через `karox update`.
- **Единая диагностика:** `karox doctor` одинаково работает на Windows, macOS и Linux.
- **Безопасный support bundle:** `karox support` создаёт ZIP без исходного кода и очищает секреты.
- **Усиленный runtime:** constant-time проверка ключа, лимиты запросов, security headers, request ID, безопасные ошибки, ротация и очистка audit-логов, временная блокировка перебора ключей.
- **Кроссплатформенное качество:** Windows, macOS и Linux проверяются на Python 3.10 и 3.12, дополнительно работает CodeQL.

## Команды приложения

| Команда | Что делает |
|---|---|
| `karox` | Открывает Project Flight Deck |
| `karox version` | Показывает установленную версию |
| `karox status` | Показывает сохранённые и живые сессии |
| `karox doctor` | Запускает быструю диагностику |
| `karox doctor --deep` | Запускает полный тест API и безопасности |
| `karox update --check` | Проверяет стабильный канал обновлений |
| `karox update` | Устанавливает новый стабильный релиз |
| `karox support` | Создаёт очищенный support ZIP |
| `karox dashboard` | Открывает Control Center живой сессии |
| `karox notion` | Запускает KaroX сразу с Notion |

Для `version`, `status` и `doctor` доступен JSON-вывод. Полное описание — в [руководстве продукта](PRODUCT_GUIDE.md).

## Control Center

Каждая живая сессия предоставляет страницу:

```text
https://<адрес-текущей-сессии>/control
```

Команда `karox dashboard` сама откроет правильную сессию. Вставьте ключ, скопированный клавишей `K`, внутрь страницы. Он хранится только в `sessionStorage` текущей вкладки, не появляется в URL и удаляется кнопкой **Forget key**.

Control Center показывает репозиторий, ветку, профиль доступа, состояние задачи, изменённые файлы, Git status, реальные возможности и актуальный Mission Control.
...
Notion-агент получает специализированные инструменты для чтения проекта, изменения файлов, запуска сборки и тестов, проверки Git diff, безопасного commit и финального отчёта. Ограничения KaroX не обходятся. Подробности — в [инструкции Notion](NOTION.md).
...
- **Наблюдаемость:** статус сессий, Control Center, request ID, диагностика, обновления и support bundles.
...
- каждый защищённый endpoint требует уникальный ключ текущей сессии;
...
- audit-логи ротируются, а чувствительные значения очищаются;
- support bundle не содержит исходный код и повторно проверяет известные ключи сессий;
...
- [Полное руководство продукта](PRODUCT_GUIDE.md)
